...
class RoadNetwork:
    def __init__(self, graph, cost_calculator=None):
        self.graph = graph
    ...
    def initialize_edge_attributes(self):
        ...
        for source, target, key, data in self.graph.edges(
            ...
            travel_time = self._calculate_travel_time(data)

            # Simulated traffic level between 0 and 1
            traffic_level = random.uniform(0.0, 1.0)
    ...
    def calculate_dynamic_costs(self, cost_calculator=None):
        """
        Calculate and assign a dynamic cost to every road segment.
        """
        ...
        if self.cost_calculator is None:
            raise ValueError("A CostCalculator is required.")

        # Store maximum values for future recalculations
        ...
        for source, target, key, data in self.graph.edges(
            ...
            self.recalculate_road_cost(source, target, key)

    def recalculate_road_cost(self, source, target, key=0):
        """
        Recalculate the dynamic cost of a single road segment.
        """

        if self.cost_calculator is None:
            raise ValueError("CostCalculator has not been initialized.")

        edge_data = self.graph[source][target][key]

        normalized_distance = self.cost_calculator.normalize(
            edge_data.get("length", 0),
            self.max_distance
        )

        normalized_time = self.cost_calculator.normalize(
            edge_data.get("travel_time", 0),
            self.max_travel_time
        )

        dynamic_cost = self.cost_calculator.calculate_cost(
            normalized_distance=normalized_distance,
            normalized_time=normalized_time,
            traffic_level=edge_data.get("traffic_level", 0),
            risk_level=edge_data.get("risk_level", 0)
        )

        edge_data["dynamic_cost"] = dynamic_cost

    def update_road_risk(self, source, target, key, risk_level):
        """
        Update the risk level of a road and recalculate its cost.
        """
        ...
        edge_data["risk_level"] = risk_level

        self.recalculate_road_cost(source, target, key)

    def block_road(self, source, target, key=0):
        """
    ...
    def unblock_road(self, source, target, key=0):
        """
    ...
    def _calculate_travel_time(self, edge_data):
        """
        ...
        speed_kmh = self._get_speed(edge_data)
    ...
    def _get_speed(self, edge_data):
        ...
        if maxspeed:
            return self._parse_speed(maxspeed)
        ...
        return fallback_speeds.get(highway_type, 30)

    def _parse_speed(self, maxspeed):
        """
    ...
    def validate_edge_attributes(self):
        """
        Validate that all roads contain the required disaster-aware attributes.
        """
